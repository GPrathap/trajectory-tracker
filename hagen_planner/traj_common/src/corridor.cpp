...
namespace hagen_planner{
    ...
    Corridor::Corridor(vec_E<Polyhedron<3>> polyhedrons){
    ...
    void Corridor::FindPolygon(const Eigen::Vector3d position, const Eigen::Vector3d tangent_line, const int corridor_id)
    {
        clock_t t1, t2;
        t1 = clock();
       if((Cn[corridor_id] * position - dn[corridor_id]).maxCoeff() >= 0){
            std::cout<< "position should be inside: " <<  (Cn[corridor_id] * position - dn[corridor_id]).maxCoeff()   << std::endl;
            return;
       }
        ...
        */
        ...
        hagen_planner::Polyhedron1 polyhedron(final_line_id_table.size(),4);
