...
private:
	...
	inline bool checkOccupancy(const Eigen::Vector3d &pos) { 
		if(pos[2] <= grid_map_->sdf_map_->mapMinAlt()){
			// ROS_ERROR("Not allowed to plan, below %f %f %f", pos(0), pos(1), pos(2));
			return true;
		}
		if(pos[2] >= grid_map_->sdf_map_->mapHighAlt()){
			// ROS_ERROR("Not allowed to plan, below %f %f %f", pos(0), pos(1), pos(2));
			return true;
		}
...
public:
	...
	bool could_not_find_path = false;
	double max_time_limit = 20.0;
	double step_time_search = 2.0;
	double current_time_step = 1.0;
};
...
inline bool AStar::Coord2Index(const Eigen::Vector3d &pt, Eigen::Vector3i &idx) const
{
	...
	if(!grid_map_->is_inside_map(pt)){
		ROS_ERROR("Expected pose out of map, index=%d %d %d", idx(0), idx(1), idx(2));
		return false;
	}
	return true;
};
