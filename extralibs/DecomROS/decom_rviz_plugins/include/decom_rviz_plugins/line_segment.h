...
#include <decom_rviz_plugins/decomp_base.h>
...
struct free_space_rep{
      Ellipsoid3D elliposids_;
      Polyhedron3D polyhedron_;
};

typedef struct free_space_rep free_info;
...
class LineSegment : public DecompBase<Dim> {
  public:
    ...
    free_info dilate_parallel(){
      find_ellipsoid(0.0);
      this->find_polyhedron();
      add_local_bbox(this->polyhedron_);
      free_info free_space_info;
      free_space_info.elliposids_ = this->ellipsoid_;
      free_space_info.polyhedron_ = this->polyhedron_;
    ...
    template<int U = Dim>
      ...
      find_ellipsoid(double offset_x) {
        ...
        while (!obs_inside.empty()) {
          ...
          obs_inside = obs_new;
          if(obs_inside.size()==1){
            if(last__set==false){
                last_p =  obs_inside[0];
                last__set = true;
            }else{
              auto diff = (obs_inside[0] - last_p).norm();
              last_p =  obs_inside[0];
              if(diff<0.05){
                std::cout<< "Sampe point, "<< std::endl;
                break;
              }
            }
          }
      ...
      while (!obs_inside.empty()) {
        const auto pw = E.closest_point(obs_inside);
        Eigen::Matrix<decimal_t, 3, 1> p = Rf.transpose() * (pw - E.d());
