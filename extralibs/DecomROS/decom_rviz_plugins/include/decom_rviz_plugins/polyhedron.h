...
struct LinearConstraint {
  ...
  LinearConstraint(const MatDNf<Dim>& A, const Eigen::Matrix<decimal_t, Eigen::Dynamic, 1>& b) : A_(A), b_(b) {}
  /**
   ...
   */
	LinearConstraint(const Vecf<Dim> p0, const vec_E<Hyperplane<Dim>>& vs) {
		...
		Eigen::Matrix<decimal_t, Eigen::Dynamic, 1> b(size);
  ...
  bool inside(const Vecf<Dim> &pt) {
    Eigen::Matrix<decimal_t, Eigen::Dynamic, 1> d = A_ * pt - b_;
  ...
  /// Get \f$b\f$ matrix
  Eigen::Matrix<decimal_t, Eigen::Dynamic, 1> b() const { return b_; }

  MatDNf<Dim> A_;
  Eigen::Matrix<decimal_t, Eigen::Dynamic, 1> b_;
};
