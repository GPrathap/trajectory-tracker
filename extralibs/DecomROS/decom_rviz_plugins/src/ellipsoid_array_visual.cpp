...
namespace decom_rviz_plugins {
  ...
  void EllipsoidArrayVisual::setMessage(const decom_ros_msgs::EllipsoidArray::ConstPtr &msg) {
    objs_.clear();
    ...
    int cnt = 0;
    for (const auto &it : msg->ellipsoids) {
      Eigen::Matrix<decimal_t, 3, 3> E;
      for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++)
          E(i, j) = it.E[3 * i + j];
      Eigen::SelfAdjointEigenSolver<Eigen::Matrix<decimal_t, 3, 3>> es(E);
