...
namespace decom_rviz_plugins {
  ...
  void VectorVisual::setMessage(const vec_E<std::pair<Eigen::Matrix<decimal_t, 3, 1>, Eigen::Matrix<decimal_t, 3, 1>>> &vs) {
    objs_.clear();
    vs_ = vs;
    if (vs.empty())
      return;
    ...
    int cnt = 0;
    for (const auto &v : vs) {
      Eigen::Matrix<decimal_t, 3, 1> n = v.second.normalized();
