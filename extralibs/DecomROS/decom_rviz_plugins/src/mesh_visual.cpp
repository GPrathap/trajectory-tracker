...
namespace decom_rviz_plugins {
  ...
  void MeshVisual::setMessage(const vec_E<std::vector<Eigen::Matrix<decimal_t, 3, 1>, Eigen::aligned_allocator<Eigen::Matrix<decimal_t, 3, 1>>>> &bds) {
    obj_->clear();
    ...
    for (const auto &vs: bds) {
      if (vs.size() > 2) {
        Eigen::Matrix<decimal_t, 3, 1> p0 = vs[0];
        Eigen::Matrix<decimal_t, 3, 1> p1 = vs[1];
        Eigen::Matrix<decimal_t, 3, 1> p2 = vs[2];
        Eigen::Matrix<decimal_t, 3, 1> n = (p2-p0).cross(p1-p0);
        n = n.normalized();
        if(std::isnan(n(0)))
          n = Eigen::Matrix<decimal_t, 3, 1>(0, 0, -1);
