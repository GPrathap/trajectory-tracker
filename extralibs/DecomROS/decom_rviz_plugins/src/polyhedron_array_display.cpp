...
void PolyhedronArrayDisplay::processMessage(const decom_ros_msgs::PolyhedronArray::ConstPtr &msg) {
  ...
  for(const auto& polyhedron: polys){
    vec_E<std::vector<Eigen::Matrix<decimal_t, 3, 1>, Eigen::aligned_allocator<Eigen::Matrix<decimal_t, 3, 1>>>> bds = cal_vertices(polyhedron);
...
PLUGINLIB_EXPORT_CLASS(decom_rviz_plugins::PolyhedronArrayDisplay, rviz::Display)
